(function (window,document) {
	...
	if (locSearch) {
		CommentNS.artUrl = AllCommentNS.artUrl = locSearch.split("?")[1].split("&")[0];
		if (locSearch.split("&")[1]) {
			CommentNS.artTitle = AllCommentNS.artTitle = decodeURI(locSearch.split("&")[1]);
		};
	}
	...
	'<div class="m-cmt-section" id="JcmtAllSection"><span class="m-cmt-section-mark">所有评论</span></div>',
		...
		loadJs(CommentNS.topHotUrl + "?encodeHtml=1&url=" + CommentNS.artUrl,function (json) {
			if (json.error || json.comments.length == 0) {
				...
				for (var i = 0; i < data.length; i++) {
					JcmtHotList.appendChild(commentDiv(data[i]));
				};
			};
		},"loadHotComments");
	}
