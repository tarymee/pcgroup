(function (window,document) {
	...
	var CommentNS = {
		...
		topicId: "",//评论id
		artUrl: undefined,//评论链接
		artTitle: "太平洋汽车网",//评论标题
	...
	if (locSearch) {
		CommentNS.artUrl = locSearch.split("?")[1].split("&")[0];
		if (locSearch.split("&")[1]) {
			CommentNS.artTitle = decodeURI(locSearch.split("&")[1]);
		};
	}

	document.title = "评论主题_" + CommentNS.artTitle;
	...
	var Jcmt = $$("Jcmt");
	var cmtHtml = [
	'<div class="m-cmt-section" id="JcmtHotSection" style="display:none;"><span class="m-cmt-section-mark">热门评论</span></div>',
	'<div class="m-cmt-list" id="JcmtHotList" style="display:none;"></div>',
	'<div class="m-cmt-section"><span class="m-cmt-section-mark">所有评论</span></div>',
	...
	'<div class="m-cmt-loading" id="JcmtLoading" style="display:none;"><p>正在加载中...</p></div>',
	'<div class="m-cmt-bg" id="JcmtBg" style="display:none;"></div>',
	'<div class="m-cmt-post" id="JcmtPost" style="display:none;">',
	...
	'	</div>',
	'	<div class="m-cmt-post-emo-list m-cmt-post-emo-list-dn" id="JcmtFaceList"></div>',
	'</div>',
	'<div class="m-cmt-nav" id="JcmtNav" style="display:none;"></div>',
	'<div class="m-cmt-tf" id="JcmtTf">',
	'<div class="m-cmt-tf-con">',
	'	<div class="m-cmt-tf-input" id="JcmtPostOpen">我来说两句...</div>',
	'	<div class="m-cmt-tf-nav-btn" id="JcmtNavBtn" style="display:none;"></div>',
	'</div>',
	'</div>'].join("");
	...
	var bottomDivEle = document.createElement("div");
	bottomDivEle.className = "m-cmt-bottom-space";
	document.getElementsByTagName("body")[0].appendChild(bottomDivEle);
	...
	if (typeof(CommentNS.navData) != "undefined" && CommentNS.navData.length > 0) {
		...
		for (var i = 0; i < CommentNS.navData.length; i++) {
			...
			html += '<a href="' + CommentNS.navData[i].url + '" class="' + (isCur ? 'cur' : '') + '">' + CommentNS.navData[i].title + '</a>';
		};
		html += '</div>';
	...
	// 输入框聚焦时设置isonFocus为true 用来检测 优化用户体验
	var isonFocus = false;
	JcmtCon.addEventListener("focus",function(){
		isonFocus = true;
	},false);
	//+++++++++++点击显示隐藏表情 插入表情代码++++++++++++++
	JcmtFaceBtn.addEventListener("click",function(){
	...
	function handleSuccessSend(responseText) {
		...
		if (data.resultCode >= 0) {
			...
			//提交成功后隐藏表情框
			JcmtPost.querySelector(".m-cmt-post-emo-list").className = "m-cmt-post-emo-list m-cmt-post-emo-list-dn";
	...
	function getAvatar(id) {
		...
		return 'http://i6.3conline.com/images/upload/upc/face/' + ret.join('/') + '/' + id + '_50x50';
	}
		...
		loadJs(CommentNS.topHotUrl + "?encodeHtml=1&url=" + CommentNS.artUrl,function (json) {
			if (json.error || json.comments.length == 0) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					var noNameClass = data[i].userId == 0 ? "m-cmt-list-item-name-none" : "";

					html += '<div class="m-cmt-list-item" data-meta="' + data[i].floor + ',' + data[i].nickName + ',' + data[i].id + '">';
					html += '<div class="m-cmt-list-item-l">';
					html += data[i].userId != 0 ? '<a href="http://m'+CommentNS.domain+'/my/' + data[i].userId + '/">' : '';
					html += '<img src="' + data[i].face + '" />';
					html += data[i].userId != 0 ? '</a>' : '';
					html += '</div>';

					html += '<div class="m-cmt-list-item-con">';

					html += '<div class="m-cmt-list-item-th"><span class="m-cmt-list-item-name ' + clientClass + ' ' + noNameClass + '">' + data[i].nickName + '</span><span class="m-cmt-list-item-floor">' + data[i].floor + '楼</span></div>';
					html += '<div class="m-cmt-list-item-tb">';
					html += '<div class="m-cmt-list-item-body">' + data[i].content + '</div>';
					html += '</div>';
					html += '<div class="m-cmt-list-item-tf">';
					html += '<span class="m-cmt-list-item-date">' + data[i].createTime + '</span>';
		...
		loadJs(CommentNS.nextPageUrl + "?encodeHtml=1&urlHandle=1&url=" + CommentNS.artUrl + "&pageNo=" + CommentNS.pageNo + "&pageSize=" + CommentNS.pageSize, function(json) {
			if (CommentNS.pageNo == 1) { //第一次加载时
				...
				if (json.error) { //没有评论时
					...
					CommentNS.artTitle = json.title;//赋予标题
					document.title = "评论主题_" + CommentNS.artTitle;
			...
			for (var i = 0; i < data.length; i++) {
				...
				html += '<div class="m-cmt-list-item-tf">';
				html += '<span class="m-cmt-list-item-date">' + data[i].createTime + '</span>';
				html += '<span class="m-cmt-list-item-btns"><a class="m-cmt-list-item-ding" '+(("ontouchstart" in document.documentElement) ? "ontouchstart" : "onclick")+'="cmt4_vote(' + data[i].id + ',1)"><i id="support' + data[i].id + '">' + data[i].support + '</i></a><a class="m-cmt-list-item-repost">回复</a></span>';
				html += '</div>';
				html += '</div>';
				divEle.innerHTML = html;
			...
			if (CommentNS.index >= json.total) {
				JcmtMore.innerHTML = "没有条目了";
				JcmtMore.onclick = null;
			}else {
				JcmtMore.innerHTML = "点击加载更多";
			};
		}, "loadComments");
	}
	...
	function genReplyHtml(data) {
		...
		var noNameClass = data.userId == 0 ? "m-cmt-list-item-name-none" : "";
	...
	if (Cookie.get(CommentNS.cookieUserSession) && Cookie.get(CommentNS.cookieUserCmu)) {
		if (Cookie.get("logedUserName")) {
			...
			CommentNS.userId = Cookie.get("logedUserId");
			insertLogedUser();
		} else {
